...
function processDuplicateFree(list, callback) {
  const arr = [];
  return callback(list.filter(function(item){var unique = callback(item); return arr.indexOf(unique) >= 0 ? false: arr.push(unique);}));}
...
function getFullNames(runners) {
  let fullName = [];
     runners.forEach(data => {fullName.push(`${data.last_name}, ${data.first_name}`); } );
...
function firstNamesAllCaps(runners) {
  let nameList = [];
  runners.forEach(data => {nameList.push(`${data.first_name.toUpperCase()}`)});
  return nameList;
}
...
function tallyUpDonations(runners){
  let total = runners.reduce((total, runner) => {return total + runner.donation;}, 0);
  return total;
}
...
function counterMakerWithLimit(max) {
  let count = 0;
  return function counter(){
    do{count++; return count;} while (count<= max);

  };
const counter = counterMakerWithLimit(max);
counter();
}
